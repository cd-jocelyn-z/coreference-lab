...
def make_conll(doc, add_singletons, doc_id, doc_boundaries, tokens_sentence_boundaries=None):
    """
    Produce a CoNLL part (string) from a spaCy doc already parsed with coreferee.
    For plain text input:
      - If doc_boundaries is None, the function uses the entire doc.
      - If tokens_sentence_boundaries are not provided, tokens and sentence breaks are computed using spaCy's tokenization.
    """
    
    if doc_boundaries is None:
        spacy_tokens_full = doc[:]
    else:
        spacy_tokens_full = doc[doc_boundaries['start']:doc_boundaries['end']]

    

    spacy_token_root = spacy_tokens_full.root
    ...
    if tokens_sentence_boundaries:
        ...
        sentence_breaks = [s.end for s in doc.sents]
        if sentence_breaks:
            sentence_breaks = sentence_breaks[:-1]

    
    if doc_id:
        doc_name_match = re.search(r"\((.*?)\)", doc_id)
        if doc_name_match:
            doc_name = doc_name_match.group(1)
        else:
            doc_name = doc_id

        doc_part_match = re.search(r"part (\d+)", doc_id)
        if doc_part_match:
            doc_part = doc_part_match.group(1)
        else:
            doc_part = "0"
    else:
    ...
    for i, token_boundary in enumerate(tokens_boundaries):
        ...
        token_span = doc.char_span(token_start, token_end + 1, alignment_mode="expand")
        if token_span is None:
            
            token_span = doc[i:i+1]
        spacy_token_root = token_span.root
        ...
        for mention, chain_index in mentions.items():
            ...
            if token_start <= mention_start <= token_end and token_start <= mention_end < next_token_start:
                if duplicate:
                    continue
                ...
                corefs.append(f"{chain_index})")
                if chain_index in unclosed_corefs:
                    unclosed_corefs.remove(chain_index)
                
        
        if i < size_doc - 1 and any(token_end <= b < tokens_boundaries[i + 1][0] for b in sentence_breaks):
            corefs.extend(f"{chain_index})" for chain_index in unclosed_corefs)
